...
public class TpaHereRequestHandler {
    ...
    public static void handle(TpaHereRequest tpaRequest) {
        ...
        if(playerTarget == null){
            DatasyncTp.LOGGER.warn("Player " + tpaRequest.getPlayerTarget() + " not found for tpahere request");
            return;
        }
    ...
    private static void informPlayer(ServerPlayer player, ISyncPlayer playerSource) {
        Component message = Component.literal("You have received a tpahere request from " + playerSource.getName() + ". Click on buttons below to accept or deny the request or type /tpahereaccept or /tpheredeny in chat\n").withStyle(style -> style.withColor(ChatFormatting.GOLD));
    ...
    public static void accept(ServerPlayer player){
        ...
        for(TpaHereRequest tpaRequest : tpaRequestBuffer){
            if(tpaRequest.getPlayerTarget().getUUID().equals(player.getUUID())){
                ...
                new TpRequest(tpaRequest.getPlayerTarget(), tpaRequest.getPlayerSource()).fire();
